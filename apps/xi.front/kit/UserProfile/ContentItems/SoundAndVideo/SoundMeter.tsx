import { useEffect, useState } from 'react';
import { observer } from 'mobx-react';
import MediaSt from 'store/media/mediaSt';
import styled from '@emotion/styled';
import { useStore } from 'store/connect';

type SoundMeterT = {
  animate: boolean;
  userMediaSt: MediaSt;
  colorScheme?: 'light' | 'dark';
};

const SoundMeter = observer((props) => {
  const { animate, colorScheme = 'light' }: SoundMeterT = props;

  const rootStore = useStore();
  const {
    mediaSt: {
      mediaInfo: { stream },
    },
  } = rootStore;

  const [animateId, setAnimateId] = useState<number>(0);
  const [volumeValue, setVolumeValue] = useState<number>(0);

  const startSoundAnalyser = () => {
    if (animateId) cancelAnimationFrame(animateId);

    if (stream) {
      const audioContext = new AudioContext();

      const analyserNode = audioContext.createAnalyser();

      const mediaStreamAudioSourceNode = audioContext.createMediaStreamSource(stream);

      mediaStreamAudioSourceNode.connect(analyserNode);

      const pcmData = new Float32Array(analyserNode.fftSize);

      const startAnimate = () => {
        analyserNode.getFloatTimeDomainData(pcmData);

        let sumSquares = 0.0;

        for (const amplitude of pcmData) {
          sumSquares += amplitude * amplitude;
        }

        setVolumeValue(Math.sqrt(sumSquares));
        setAnimateId(requestAnimationFrame(startAnimate));
      };

      startAnimate();
    }
  };

  const SoundMeterLight = styled.meter`
    width: 100%;
    height: 20px;

    &::-webkit-meter-bar {
      background: #e6e6e6;
      border-radius: 100px;
      box-shadow: none;
      border: none;
    }

    &::-webkit-meter-optimum-value {
      background: #445aff;
    }
  `;

  const SoundMeterDark = styled.meter`
    width: 100%;
    height: 12px;

    &::-webkit-meter-bar {
      background: #333333;
      border-radius: 2px;
      box-shadow: none;
      border: none;
    }

    &::-webkit-meter-optimum-value {
      background: #445aff;
    }
  `;

  useEffect(() => {
    if (animate) startSoundAnalyser();

    return () => {
      setVolumeValue(0);
      cancelAnimationFrame(animateId);
    };
  }, [stream]);

  return colorScheme === 'light' ? (
    <SoundMeterLight value={volumeValue} />
  ) : (
    <SoundMeterDark value={volumeValue} />
  );
});

export default SoundMeter;
