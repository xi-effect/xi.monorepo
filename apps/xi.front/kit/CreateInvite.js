...
const steps = [
  ...
  },
  {
    ...
    description: 'Вы можете дополнительно задать время действия приглашения с момента его создания',
  },
  {
...
const CreateInvite = observer(() => {
    ...
    if (!errorsOnFields) {
      rootStore.socket.emit(
        ...
        ({ code, data, message }) => {
          ...
          if (data.code) {
            setTimeout(
              () => setInviteLink(`https://xieffect.ru/invite/community/${data.code}`),
              1000,
            );
          }
        },
  ...
  const generatedLinkBlock = (
    <Paper square elevation={0} sx={{ p: 3, borderRadius: 4 }}>
      <Typography>
        {inviteLink ? 'Новое приглашение только что создано!' : 'Ваша ссылка генерируется...'}
      </Typography>
      <Typography variant="subtitle2">
        {inviteLink
          ? 'Скопируйте ссылку ниже и отправьте её пользователю, которого хотите пригласить'
